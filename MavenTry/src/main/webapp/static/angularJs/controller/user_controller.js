...
//					$scope.items =  angular.copy(self.user);
					...
					$log.info("FALSE model ", $scope.items);
...
var ModalInstanceCtrlv1 = function($scope, $uibModalInstance, userForm, $log, User) {
...
App.controller('ModalInstance', function($scope, $uibModalInstance, items,
	...
	//$scope.userInfo.userProfiles[0].id = $scope.userInfo.userProfiles[0].id;
	
	$scope.submitForm = function() {
		if ($scope.form.userForm.$valid) {
			
			//console.log("FORM SELECTED "+ $scope.form.userForm.userInfo.userProfiles.id);
...
//			$log.info("ID test", myModal.user.id);
			...
			if (myModal.user.id == "") {
				...
				$log.info("NULL myModal.user test ", myModal.user);
			 ...
			 myModal.createUser();
				...
				$log.info("NOT NULL myModal.user test ", myModal.user);

				myModal.user.userProfiles[0].id = $scope.userInfo.userProfiles.id;
						
				$log.info("ID test", myModal.user.userProfiles[0].id);
